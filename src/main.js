...
export default function NetVisionDashboard() {
  ...
  function restoreRuntimeData() {
    loadDashboardData().then((payload) => { setData(payload); setTimeIndex(0); setImportState({ status: 'idle', fileName: '', importType: 'reference', preview: null, result: null, error: '' }); fetch('/api/recommend-context', { method: 'DELETE' }).catch(() => {}) }).catch((err) => setLoadError(err.message || String(err)))
  }
  ...
  else panel = <CockpitPanel activeTab={activeTab} scope={scope} data={data} nationalSummary={nationalSummary} governorateSummary={governorateSummary} delegationSummary={delegationSummary} summary={currentSummary} governorateRows={governorateRows} delegationRows={delegationRows} selectedGovernorate={selectedGovernorate} selectedDelegation={selectedDelegation} selectedCell={selectedCell} siteRows={siteRows} scopedCells={scopedCells} alerts={alerts} metric={metric} currentTime={data.currentTimeEntry} filters={filters} onFilterChange={updateFilters} bands={bands} onSelectGovernorate={selectGovernorate} onSelectDelegation={selectDelegation} onSelectCell={selectCell} reconciliation={data.reconciliation} driftStatus={drift.status} driftAlerts={drift.alerts} peakRows={scopedPeakRows} backendHealth={backendHealth} workerState={workerState} importState={importState} onImportFile={handleImportFile} onImportTypeChange={(importType) => setImportState((prev) => ({ ...prev, importType }))} onRestoreRuntime={restoreRuntimeData} onExportJson={exportScopedJson} onExportReport={exportReport} />
  ...
  return (
    <div className={`app-shell ${focusMode ? 'focus-mode' : ''} ${theme === 'dark' ? 'theme-dark' : ''}`}>
      ...
      <main className="command-layout cockpit-layout">
        ...
        <section className="map-column">
          ...
          <div className="scope-footer"><span>Scope: {scope.level}</span><span>{currentSummary.observed_cells || 0} scoped cells</span><span>{data?.currentTimeEntry?.timestamp || 'No time slice'}</span><button onClick={() => setLayerVisibility((v) => ({ ...v, sites: !v.sites }))}>Sites {layerVisibility.sites ? 'on' : 'off'}</button><button onClick={() => setLayerVisibility((v) => ({ ...v, delegations: !v.delegations }))}>Delegations {layerVisibility.delegations ? 'on' : 'off'}</button></div>
        </section>
