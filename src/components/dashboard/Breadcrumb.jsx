export default function Breadcrumb({ scope, onNational, onGovernorate, onDelegation }) {
  return (
    <nav className="breadcrumb" aria-label="Scope breadcrumb">
      ...
      {scope.governorateName ? <><span>/</span><button data-testid="crumb-governorate" aria-current={scope.level === 'governorate' ? 'page' : undefined} onClick={onGovernorate}>{scope.governorateName}</button></> : <><span>/</span><strong aria-current="page">National View</strong></>}
