...
export default function RecommendationCard({ recommendation, onSimulate }) {
  ...
  return (
    <div className="recommendation-card">
      ...
      {canSimulate && action ? <button onClick={() => onSimulate(action)}>Simuler : {actionLabel}</button> : <em className="advisory-note">Conseil uniquement, pas d action simulateur disponible.</em>}
    </div>
  )
}
