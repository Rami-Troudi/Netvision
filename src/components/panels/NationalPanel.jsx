  ...
  return (
    <section className="panel-shell">
      ...
      <div className="kpi-grid">
        ...
        <KpiCard label="Délégations impactées" value={summary.delegations} />
      </div>
      ...
      <RankingTable title="Gouvernorats les plus impactés" rows={governorates.slice(0, 8)} metricLabel={metric.label} metricUnit={metric.unit} onSelect={onSelectGovernorate} />
      ...
      <button className="primary-cta" onClick={() => governorates[0] && onSelectGovernorate(governorates[0])}>Inspecter le gouvernorat le plus impacté</button>
    </section>
  )
}
...
function DelegationVariationCard({ rows = [], metric }) {
  if (!rows.length) {
    return <div className="variation-card idle"><div className="section-title">Variations délégations</div><span>Lancez la lecture ou avancez d’une tranche pour voir les délégations qui changent.</span></div>
  }
  return (
    <div className="variation-card">
      <div className="section-title">Variations délégations <span>depuis la tranche précédente</span></div>
